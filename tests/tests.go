...
func decode_pass(enpass string, key []byte) {
	...
	plaintext, err := gcm.Open(nil, []byte(nonce), []byte(enpass), nil)
	if err != nil {
		fmt.Println(err)
	}
...
func main() {
	...
	var enpass string = encode_pass([]byte("hello world"), key)
	decode_pass(enpass, key)
}
